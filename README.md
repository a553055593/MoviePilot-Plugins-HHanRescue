...
- 池基线 A 自动读取你自己的 `mybonus.php`「基本奖励」行（带你的站点 Cookie），数值因人而异，与站方结算口径一致
...
## 数值口径（v3.1.0 模型，常数全部出自 wiki 规则原文）

```
A_i   = (1 − 10^(−周数/8)) × GB × (1 + √2×10^(−(当前做种人数−1)/9))
B(池) = 25 × (2/π) × arctan(A池/300 − 5) + 20
净增量 Δ = [B(池+i)×加权倍率_new − B(池)×池倍率] × 24
档位（下载瞬间人数锁定）：≤1人→憨豆3×/积分2×；2-3人→2×/1.75×；4-5人→1.5×/1.5×
```
